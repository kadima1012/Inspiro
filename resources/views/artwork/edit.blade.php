    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-lg p-8">
                <form action="{{ route('artwork.update', $artwork->idArt) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="art_Title" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Title') }}</label>
                        <input id="art_Title" type="text" name="art_Title" value="{{ $artwork->art_Title }}" required autofocus class="w-full rounded-lg border-slate-300 focus:border-amber-500 focus:ring-amber-500 transition-all duration-300">
                    </div>

                    <div>
                        <label for="art_Description" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Description') }}</label>
                        <textarea id="art_Description" name="art_Description" rows="4" required class="w-full rounded-lg border-slate-300 focus:border-amber-500 focus:ring-amber-500 transition-all duration-300">{{ $artwork->art_Description }}</textarea>
                    </div>

                    <div>
                        <label for="art_creation_date" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Creation Date') }}</label>
                        <input id="art_creation_date" type="date" name="art_creation_date" value="{{ $artwork->art_creation_date }}" required class="w-full rounded-lg border-slate-300 focus:border-amber-500 focus:ring-amber-500 transition-all duration-300">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="art_quantity" type="number" name="art_quantity" value="{{ $artwork->art_quantity }}" required min="1" class="w-full rounded-lg border-slate-300 focus:border-amber-500 focus:ring-amber-500 transition-all duration-300">
                    </div>

                    <div>
                        <label for="filepath" class="block text-sm font-medium text-slate-700 mb-1">{{ __('Artwork Image') }}</label>
                        <input id="filepath" type="file" name="filepath" class="w-full rounded-lg border-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 transition-all duration-300">
                        @if ($artwork->filepath)
                            <div class="mt-3">
                                <img src="{{ \App\Helpers\StorageHelper::customUrl($artwork->filepath) }}" alt="{{ $artwork->art_Title }}" class="w-32 h-32 object-cover rounded-xl">
                            </div>
                        @endif
                    </div>

        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('user.shop', ['username' => $user->user_username]) }}" class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 {{ !request('type') ? 'bg-amber-500 text-slate-900' : 'bg-white text-slate-700 hover:bg-amber-100 border border-slate-200' }}">
                        All
                    </a>
                    @foreach($artworkTypes as $type)
                        <a href="{{ route('user.shop', ['type' => $type->idArtworkType, 'username' => $user->user_username]) }}" class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 {{ request('type') == $type->idArtworkType ? 'bg-amber-500 text-slate-900' : 'bg-white text-slate-700 hover:bg-amber-100 border border-slate-200' }}">
                            {{ $type->type_name }}
                        </a>
                    @endforeach
                </div>
            ...
            @if($shopItems->isEmpty())
                <div class="text-center py-20 bg-white rounded-xl shadow-lg">
                    ...
                    <p class="text-slate-400 mt-1">This artist doesn't have any items in their shop yet.</p>
                </div>
            @endif
        </div>
    </div>
